...
{
        return vec2(p.y+1.0,0);
}

// http://www.iquilezles.org/www/articles/distfunctions/distfunctions.htm

vec2 sdfSphere(in vec3 p)
{
...
{
	//return obj_floor(p);
        return obj_union(obj_floor(p), sdfSphere(p));
  ...
  //return obj_union(obj_floor(p), op_union(obj_round_box(p), sdfSphere(p)));
  ...
  //return obj_union(obj_floor(p), op_blend(p, obj_round_box(p), obj_torus(p)));
        //return obj_union(obj_floor(p), op_sub(obj_round_box(p), sdfSphere(p)));
...
void main()
{
        //vec2 vPos=-1.0+2.0*(resolution.x/resolution.y);
	fragColour=vec4(0.0);
	vec2 q = uv;
        vec2 vPos = -1.0 + 2.0 * q;
        ...
        //vec3 prp=vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*0.01; //Trackball style camera pos
